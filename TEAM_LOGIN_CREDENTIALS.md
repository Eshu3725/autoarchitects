# 🔐 AutoArchitects ATV Club - Team Member Login Credentials

**Attendance Management System Login Information**

---

## 🌐 Login URL

**Website:** http://localhost:8081/login  
*(Replace with production URL when deployed)*

---

## 👥 Team Member Credentials

All team members can login using:
- **Email:** Your registered email address
- **Password:** Your USN (University Serial Number)

---

## 📋 Complete Credentials List

### **STEERING TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Kushal N.S | kushalns32@gmail.com | 1SI22ME021 |
| Skanda Moudgalya KS | skandaksmoudgalya@gmail.com | 1SI22ME052 |
| Siddharth S. | siddharth.s7060@gmail.com | 1SI23ME093 |
| Veeresh | veereshsg14@gmail.com | 1SI23ME108 |
| Dhanush gowda | dhanushgowdaa@gmail.com | 1SI24ME025 |
| Inchara M.K | incharakswamy31@gmail.com | 1SI24ME039 |

---

### **TRANSMISSION TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Karthik K | karthikkantharaju28@gmail.com | 1SI22ME020 |
| Damaresh R | damaresh@gmail.com | 1SI23ME014 |
| Darshan H.S | darshanvaibhav2@gmail.com | 1SI23ME016 |
| Sharadhi Simha Chi Na | sharadhisimhachina@gmail.com | 1SI23ME084 |
| Shivaprakash H B | shivaprakashb712@gmail.com | 1SI23ME089 |
| Dimple K | dimplek5936@gmail.com | 1SI24ME028 |
| Karthik S | karthikkarthik19370@gmail.com | 1SI24ME046 |
| Pavan H.S | pavanshekar0206@gmail.com | 1SI24ME080 |

---

### **SUSPENSION TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Likith H | likhith.785@gmail.com | 1SI22ME023 |
| Prathik Jain T.N | prathikjaintn12@gmail.com | 1SI23ME000 |
| Mithan Yadav H.R | mithunyadavhr@gmail.com | 1SI23ME057 |
| Vivek J | vivek838466@gmail.com | 1SI23ME111 |
| Jaswanth D. | jaswanthd150@gmail.com | 1SI24ME041 |
| Manoj G | manojyadav.girish@gmail.com | 1SI24ME061 |

---

### **BRAKES TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Vivek Hiresomannavar | vivekhiresomannavar@gmail.com | 1SI22ME067 |
| Basavaraj L Arakeri | basavarajarakeri@zohomail.in | 1SI24ME008 |
| Sumanth Honnungar | sumanthhonnungar@gmail.com | 1SI23ME100 |
| Kavana G | kavanag720@gmail.com | 1SI24ME048 |
| Shashi Kumar | shashi60835@gmail.com | 1SI24CH045 |

---

### **CAE (CHASSIS) TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Yashas M.S | yashasms9125@gmail.com | 1SI23ME115 |
| Medha D. | dmedha2005@gmail.com | 1SI23ME055 |
| Mohammed Umar Siddiq | mdumarsiddiq5@gmail.com | 1SI23ME059 |
| Kamalesh D.R | drkamalesh397@gmail.com | 1SI23ME038 |
| Nikshith J | nikshithjagadeesh@gmail.com | 1SI24ME075 |
| Pallav B | pallavpallu806@gmail.com | 1SI24ME078 |

---

### **GRAPHICS TEAM**

| Name | Email | Password (USN) |
|------|-------|----------------|
| Bhumika B.R | b301898@gmail.com | 1SI24CS034 |

---

## 👨‍💼 Admin Accounts

**These members have admin access (can add/edit attendance for all members):**

| Name | Role | Email | USN |
|------|------|-------|-----|
| Kushal M.V | Captain (Steering) | kushalmvkushi2@gmail.com | 1SI23ME426 |
| Tejashree P | Vice Captain (Chassis) CAE | tejashree62005@gmail.com | 1SI22ME059 |

*Admin passwords are set separately and not listed here.*

---

## 🔑 How to Login

### **Step 1: Open the Website**
Go to: http://localhost:8081/login

### **Step 2: Enter Your Credentials**
- **Email:** Your email from the table above
- **Password:** Your USN (exactly as shown)

### **Step 3: Click "Sign In"**
You'll be redirected to your personal dashboard

---

## 📊 What You Can Do

### **As a Team Member (User Role):**
- ✅ View your own attendance records
- ✅ See your attendance statistics
- ✅ Check your attendance percentage
- ❌ Cannot add or edit attendance records
- ❌ Cannot view other members' attendance

### **As an Admin:**
- ✅ View all team members' attendance
- ✅ Add new attendance records for any member
- ✅ Edit existing attendance records
- ✅ View overall team statistics
- ✅ Manage all attendance data

---

## 🆘 Troubleshooting

### **Problem: "Invalid email or password"**

**Solutions:**
1. ✅ Check your email is typed correctly (no spaces)
2. ✅ Check your USN is typed correctly (case-sensitive)
3. ✅ Make sure you're using the exact USN from the table
4. ✅ Try copying and pasting your credentials

**Example:**
- ❌ Wrong: `1si22me021` (lowercase)
- ✅ Correct: `1SI22ME021` (uppercase)

### **Problem: "Can't access the website"**

**Solutions:**
1. ✅ Make sure the dev server is running
2. ✅ Check the URL is correct
3. ✅ Try refreshing the page (Ctrl+R or Cmd+R)
4. ✅ Clear browser cache and try again

### **Problem: "Redirected to wrong dashboard"**

**Solutions:**
1. ✅ Logout and login again
2. ✅ Clear browser cookies
3. ✅ Contact admin to verify your role

---

## 🔒 Security Notes

### **Important:**
- ⚠️ Your password is currently your USN
- ⚠️ Do NOT share your credentials with others
- ⚠️ Keep your password secure
- ⚠️ Future update will allow password changes

### **For Admins:**
- 🔐 Admin credentials should be kept strictly confidential
- 🔐 Only admins can modify attendance records
- 🔐 Admins are responsible for data accuracy

---

## 📞 Support

### **Need Help?**

**Contact:**
- **Captain:** Kushal M.V - kushalmvkushi2@gmail.com
- **Lead:** Tejashree P - tejashree3005@gmail.com
- **Digital:** Eshaan AV - eshaanvenkatesh3725@gmail.com

**Common Issues:**
- Login problems
- Password issues
- Dashboard access
- Attendance queries

---

## 📝 Quick Reference

**Total Team Members:** 34
**User Accounts:** 32
**Admin Accounts:** 2
**Password Format:** USN (e.g., 1SI22ME021)
**Login URL:** http://localhost:8082/login

**Team Breakdown:**
- Steering: 6 members
- Transmission: 8 members
- Suspension: 6 members
- Brakes: 5 members
- Chassis (CAE): 6 members
- Graphics: 1 member

---

## ✅ Account Status

All 32 team member accounts have been created and are ready to use!

**To verify your account:**
1. Go to the login page
2. Enter your email and USN
3. Click "Sign In"
4. You should see your dashboard

---

## 🎯 Next Steps

1. **Test your login** - Make sure you can access your dashboard
2. **Check your attendance** - View your personal records
3. **Report issues** - Contact admins if you have problems
4. **Keep credentials safe** - Don't share your password

---

**Welcome to the AutoArchitects ATV Club Attendance Management System!** 🚀

*Last Updated: 2025-11-11*

